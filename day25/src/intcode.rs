...
impl ProcessState {
    ...
    pub fn needs_input(&self) -> bool {
        // FIXME: probably wrong in some cases
        self.needs_input
    }
            ...
            match op {
                ...
                3 => {
                    match self.input.pop_front() {
                        Some(i) => self.read_write_mem(mode_1, self.iptr + 1, Some(i)),
                        None => {
                            self.needs_input = true;
                            // don't further the pointer if stuck on read_input
                            if self.verbose {
                                println!("warning, need input");
                            }
                            break;
                        }
                    };
